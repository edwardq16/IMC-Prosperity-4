...
df = pd.read_csv("prices_round_1_day_-2.csv", sep=";")
...
aco_clean["smoothed"] = aco_clean["mid_price"].rolling(window=500).mean()
...
plt.plot(aco_clean["timestamp"], aco_clean["smoothed"])
plt.ylim(9950, 10050)
plt.show()